# Importar el módulo subprocess para ejecutar comandos en el sistema operativo
import subprocess

# Definir códigos de colores para mensajes en la consola
green_colour = "\033[0;32m\033[1m"
end_colour = "\033[0m"
red_colour = "\033[0;31m\033[1m"

# Importar módulos para manejar señales y obtener direcciones IP
import signal
import sys
import socket

# Función para manejar la señal Ctrl+C y salir del programa de manera segura
def ctrl_c(signal, frame):
    print(f"\n\n\t{red_colour}[!] Saliendo inesperadamente ...{end_colour}")
    sys.exit(1)

signal.signal(signal.SIGINT, ctrl_c)

# Función para obtener la dirección IP del host y definir un rango de direcciones IP para el escaneo
def obtener_ip():
    ip = socket.gethostbyname(socket.gethostname()).rsplit('.', 1)[0] + ".1/24"
    return ip

# Función para escanear puertos utilizando la herramienta Nmap
def escanear_puertos():
    # Obtener la dirección IP
    ip = obtener_ip()

    # Validar la dirección IP antes de usarla
    try:
        socket.inet_aton(ip.split('/')[0])
    except socket.error:
        print(f"\n{red_colour}[!] Error: Dirección IP no válida.{end_colour}\n")
        sys.exit(1)

    # Ejecutar el comando Nmap con los parámetros específicos
    nmap_process = subprocess.Popen(['nmap', '--top-ports', '500', '--open', ip], stdout=subprocess.PIPE)
    
    # Obtener y decodificar la salida del proceso Nmap
    output = nmap_process.communicate()[0].decode('utf-8')
    
    # Imprimir la salida del escaneo
    print(output)

# Llamar a la función para ejecutar el escaneo de puertos
escanear_puertos()
